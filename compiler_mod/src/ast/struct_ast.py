...
def helper_get_item_from_struct(struct_get,id_entry,env):
    value = struct_get("struct_env").get(id_entry)
    value = ic(struct_get("struct_env"))
    if value is None:
...
class StructExpression(InterpretedExpression):
    entries: List
    def eval(self,env:Env,is_struct):
        is_struct = True
        # struct_env = Env(env)
        struct_env = env.deep_copy()
        ic("=============h1==================")
        ic("=============h1==================")
        ic(struct_env)
        ...
        # struct_env.struct_dict["env"] = struct_env
        def struct_get(val):
            ic("=============START EVAL STRUCT==================")
            ic(val)
            ic(struct_env)
            ic(is_struct)
            ic(struct_env.getitem_if_struct_from_struct(val,is_struct))
            res = struct_env.getitem_if_struct_from_struct(val,is_struct)
            # res = struct_env.struct_dict.get(val,None)
            # res = struct_env.get_struct_dict_entry(val)
            # ic(res)
            # ic(struct_env)
            ic("=============h4==================")
...
class StructVariableFromOutside(InterpretedExpression):
    ...
    def eval(self, env: Env, is_struct):
        ...
        ic(id_struct_or_parent_struct_get)
        ic(id_struct_or_parent_struct_get(self.id_entry))
        # exit()
        struct_entry = helper_get_item_from_struct(id_struct_or_parent_struct_get,self.id_entry,env)
        res = struct_entry[self.id_entry]
        # res = res.eval(env,is_struct)[0]
        res = res
        # exit()
        # exit()
        return res, env
...
class StructCallFunctionFromOutside(InterpretedExpression):
    ...
    def eval(self, env: Env, is_struct):
        ...
        id_struct_or_parent_struct_get = get_struct_x_parent_or_self_struct(self.id_struct,self.dots_count,env)
        ic(id_struct_or_parent_struct_get)
        struct_entry_func = helper_get_item_from_struct(id_struct_or_parent_struct_get,self.id_entry,env)
        ic(struct_entry_func)

        ic(self.id_entry)
        ic(self.args_function)
        ic(struct_entry_func)
        ic(struct_entry_func[self.id_entry])
        ic(struct_entry_func[self.id_entry](self.args_function))
        # ic(struct_entry_func[self.id_entry].eval(env,is_struct)[0])
        # ic(struct_entry_func[self.id_entry].eval(env,is_struct)[0](self.args_function))
        # ic(struct_entry_func[self.id_entry])
        # res = struct_entry_func[self.id_entry].eval(env,is_struct)[0](self.args_function)
        # ic(res)
        res = struct_entry_func[self.id_entry](self.args_function)
        # exit()
        # exit()
        # res = struct_entry_func[self.id_entry](self.args_function)
        # ic(self.args_function)
