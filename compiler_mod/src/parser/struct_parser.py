...
def p_expression_assign_struct_write_id(p):
    ...
    '''
    # if len(p)== 4:
        # p[0] = Node(Expr.StructVariableFromOutside,[p[1],p[3]])
        # p[0] = p[1],p[3]
    # else:
        # p[0]=p[1]
    # p[0] = [p[2],p[4]]
...
def p_expression_lambda__in_struct(p):
    ...
    '''
    # '''lambda_expression_struct :   LAMBDA_START expression_list  LAMBDA "."  expression
    #                     |           LAMBDA_START LAMBDA "." expression
    # '''
    if len(p) == 5:
        p[0] = Node(Expr.LambdaArgsExpression,[[], p[4]])
    if len(p) == 6:
        p[0] = Node(Expr.LambdaArgsExpression,[p[2], p[5]])
    ic("=============h22==================")
    ic(p[0])
